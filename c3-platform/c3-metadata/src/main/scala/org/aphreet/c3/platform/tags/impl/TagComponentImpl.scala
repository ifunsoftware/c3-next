...
trait TagComponentImpl extends TagComponent {
  ...
  val tagManager = new TagManagerImpl
    ...
    override def deleteResource(resource:Resource) {
      resource.metadata(TagManager.TAGS_FIELD).foreach(tagsString => {
        ...
        if (isDirectory(resource)) {
          this ! DeleteParentTagMsg(resource.systemMetadata(Node.NODE_FIELD_PARENT), parseTagsField(tagsString))
        } else {
          val tagCollection = resource.metadata.collectionValue(TagManager.TAGS_FIELD)
          this ! DeleteParentTagMsg(resource.systemMetadata(Node.NODE_FIELD_PARENT), createIdentityMap(tagCollection))
        }
      })
    }
        ...
        react{
          ...
          case ResourceAddedMsg(resource, source) => {
            ...
            metadata(TagManager.TAGS_FIELD) match {
              ...
              case Some(tagsS) =>
                ...
                if (isDirectory(resource)) {
                  this ! AddParentTagMsg(resource.systemMetadata(Node.NODE_FIELD_PARENT), parseTagsField(tagsS))
                } else {
                  val tagCollection = metadata.collectionValue(TagManager.TAGS_FIELD)

                  this ! AddParentTagMsg(resource.systemMetadata(Node.NODE_FIELD_PARENT), createIdentityMap(tagCollection))
                }
            }
          }
          ...
          case ResourceUpdatedMsg(resource, source) => {
            resource.systemMetadata(Node.NODE_FIELD_PARENT)
              .foreach(value => this ! RebuildParentTagMsg(Some(value)))
          }
